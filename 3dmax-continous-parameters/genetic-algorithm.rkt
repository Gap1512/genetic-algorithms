...
(require "shared.rkt" "utilities.rkt")
...
(define (genetic-algorithm #:function (fn GA-FUNCTION-3D)
  ...
  (update-population (generate-initial-population number-of-variables number-of-individuals continous? resolution range)
                       ...
                       tournament-k elitism resolution range c-mode pc m-mode pm last-generation))
                      ...
                      (cond
                        ((= actual-generation last-generation)
                         (let* ((result (remove-threshold (reverse populations) threshold mode))
                           ...
                           (values (best result fn continous? range max-value) (best-for-generation result fn) (med-for-generation result))))
                        (else
                         (begin
...
(define (best populations fn continous? range max-value)
  (let ((result (first (sort (last populations) fn #:key first))))
    (if continous?
        (append (list-tail result 1) (list (first result)))
        (append (format-individual (list-tail result 1) range max-value) (list (first result))))))
...
(define (add-fitness population function threshold continous? range max-value mode)
  (map (λ (ind)
         (let ((f-ind (if continous? ind (format-individual ind range max-value))))
           (cons (mode threshold (apply function f-ind)) ind)))
       population))
...
(define (wright-closure function threshold continous? range max-value)
  (λ (population)
    (select-best 2 (add-fitness population function threshold continous? range max-value))))
