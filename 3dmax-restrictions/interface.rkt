...
(define fn (new combo-field%
                ...
                [init-value "(λ (x y) (+ (sqr (sub1 x)) (sqr (sub1 y))))"]))
...
(define restrictions-p (new check-box%
                            [label "RESTRICTIONS"]
                            [parent setup-panel]))

(define rp (new text-field%
                        [label "RP"]
                        [parent setup-panel]
                        [init-value "50000"]))

(define equation-restriction (new text-field%
                                  [label "EQUATION RESTRICTION (= 0)"]
                                  [parent setup-panel]
                                  [init-value "(λ (x y) (sqr (- x y 2)))"]))

(define inequation-restriction (new text-field%
                                    [label "INEQUATION RESTRICTION (<= 0)"]
                                    [parent setup-panel]
                                    [init-value "(λ (x y) (sqr (max 0 (+ x y -0.5))))"]))
...
(define start (new button%
                   ...
                   [callback (λ (b e)
                               (let ((fn (get-value fn))
                                     ...
                                     (r (get-value range))
                                     (r? (send restrictions-p get-value))
                                     (rp (get-value rp))
                                     (eq-r (get-value equation-restriction))
                                     (ineq-r (get-value inequation-restriction))
                                 ...
                                 (let-values ([(best best-for-generation med-for-generation) (time (genetic-algorithm #:function fn
                                                                                                                ...
                                                                                                                #:restrictions? r?
                                                                                                                #:rp rp
                                                                                                                #:equation-restriction eq-r
                                                                                                                #:inequation-restriction ineq-r
                                                                                                                #:c-mode c-m
                                                                                                                #:pc pc
                                                                                                                #:m-mode m-m
                                                                                                                #:pm pm
